...
# All NJ counties, but only examining ethnic_minority, low_income, and racial_minority.
...
ipd_year <- 2018
ipd_states <- c("NJ")
ipd_counties <- c("34001", "34003", "34005", "34007", "34009", "34011", "34013", "34015", "34017",
...
percs <- c(ethnic_minority_percent,
           low_income_percent,
           racial_minority_percent)

percs_ids <- c("EM_P", "LI_P", "RM_P")
...
export_means <- dl_counts %>% select(GEOID10, ends_with("UE"), ends_with("CE")) %>%
  ...
  select(-GEOID10) %>%
  group_by(County) %>%
  summarize(EM_PctEst = sum(EM_CE) / sum(EM_UE),
