...
for (c in 1:length(comp$uni_est)){
  p <- unlist(comp$pct_est[,c])
  breaks <- st_dev_breaks(p, 3, na.rm = TRUE)
  score <- case_when(p < breaks[2] ~ 0,
                     p >= breaks[2] & p < breaks[3] ~ 1,
                     p >= breaks[3] ~ 2)
  class <- case_when(score == 0 ~ "Below Average",
                     score == 1 ~ "Average",
...
ipd$STATEFP10 <- str_sub(ipd$GEOID10,1,2) 
ipd$COUNTYFP10 <- str_sub(ipd$GEOID10,3,5) 
ipd$NAME10 <- str_sub(ipd$GEOID10,6,11) 
...
ipd <- ipd %>% rename(GEOID20 = GEOID10, STATEFP20 = STATEFP10, COUNTYFP20 = COUNTYFP10, NAME20 = NAME10)

st_write(trct, here("outputs", "ipd_3binSD.shp"), delete_dsn = TRUE, quiet = TRUE)
write_csv(ipd, here("outputs", "ipd_3binSD.csv"))
write_csv(export_counts, here("outputs", "counts_by_indicator_3binSD.csv"))
write_csv(export_breaks, here("outputs", "breaks_by_indicator_3binSD.csv"))
write_csv(export_summary, here("outputs", "summary_by_indicator_3binSD.csv"))
write_csv(export_means, here("outputs", "mean_by_county_3binSD.csv"))
