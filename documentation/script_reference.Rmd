...
Placeholder if you have never installed an API key before. If this is you, check out `getting_started.pdf` for where to get a Census API key and how to install it.
```{r api_key}
# Census API Key
...
The chunk below zips the user-defined calls from the API with the preferred script abbreviations into a tibble called `counts_calls` and separates the calls into three batches.
...
```
API calls are made separately for ACS Subject Tables, Detailed Tables, and Data Profiles and appended to `dl_counts`. Sometimes there are no requests for a certain type of table; in these situations, the script bypasses a download attempt. Then, information from `counts_calls` is used to rename the downloads to the appropriate abbreviation.
...
Subset `dl_counts` and `dl_percs` for DVRPC's nine-county region. Percentages should be in the range 0 to 1, so all values in `dl_percs` are divided by 100.
...
If percentage MOE == 0, then make MOE = 0.1. This should be a rare occurence with survey data at the census tract level.
```{r perc_excp_4}
