...
older_adults_percent                 <- "S0101_C02_028"
...
web <- read_csv(here("compare_2015", "full_ipd.csv")) %>%
  mutate_at(vars("GEOID"), as.character)
new_data <- ipd
names(new_data) <- paste(names(new_data), "n", sep = "_")
compare <- left_join(web, new_data, by = c("GEOID" = "GEOID_n")) %>%
  select(GEOID, sort(current_vars()))
compare_estimates <- compare %>% select(GEOID, contains("PctEst"))
compare_counts <- compare %>% select(GEOID, contains("CntEst"))
compare_moes <- compare %>% select(GEOID, contains("PctMOE"))
compare_cnt_moes <- compare %>% select(GEOID, contains("CntMOE"))
compare_pctile <- compare %>% select(GEOID, contains("Pctile"))
compare_score <- compare %>% select(GEOID, contains("Score"))

write_csv(compare_estimates, here("compare_2015", "test_compare_2015_pct.csv"))
write_csv(compare_counts, here("compare_2015", "test_compare_2015_count.csv"))
write_csv(compare_moes, here("compare_2015", "test_compare_2015_pctMOE.csv"))
write_csv(compare_cnt_moes, here("compare_2015", "test_compare_2015_cntMOE.csv"))
write_csv(compare_pctile, here("compare_2015", "test_compare_2015_pctile.csv"))
write_csv(compare_score, here("compare_2015", "test_compare_2015_score.csv"))
write_csv(as_tibble(break_storage), here("compare_2015", "test_compare_2015_breaks.csv"))
